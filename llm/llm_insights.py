...
def extract_funnel_metrics(data_list):
    ...
    for item in data_list:
        ...
        add_to_cart += int(item.get('add_to_cart', 0) or 0)
        initiate_checkout += int(item.get('initiate_checkout', 0) or 0)
        purchase += int(item.get('purchases', 0) or 0)
...
def compute_baseline(historical_data, campaign_age_days=None):
    ...
    for d in historical_data:
        ...
        r_val = (purchase_value / spend) if spend > 0 else 0
...
def build_llm_payload(campaign_meta, adsets_data, account_baseline_data):
    ...
    purchases = sum(int(a.get('purchases', 0) or 0) for a in adset_list)
    purchase_value = sum(float(a.get('purchase_value', 0) or 0) for a in adset_list)
    ...
    metrics = {
        ...
        "reach": reach,
        "purchases": purchases,
        "purchase_value": round_number(purchase_value),
